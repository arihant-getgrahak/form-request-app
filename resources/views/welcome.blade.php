...
<head>
    ...
    <meta name="description" content="Laravel Form with all the validation">
    <script src="https://cdn.tailwindcss.com"></script>
</head>

<body>
    <div class="flex flex-col items-center justify-center">
        <h1 class="text-3xl font-bold">Laravel Form</h1>
        <form action="{{ url('/') }}" method="post" class="space-y-4 border border-black w-1/3 p-4 mt-4">
            @csrf
            ...
            <div class="flex flex-col gap-2">
                ...
                <input type="text" name="password_confirmation" id="password_confirmation" class="border border-black p-1 rounded-md" value="{{ old('name') }}">
                @error('name')
                    <p class="text-red-500">{{ $message }}</p>
                @enderror
            </div>
            ...
            <button type="submit" class="border border-black p-1 rounded-md bg-blue-500 text-white">Submit</button>
        </form>
    </div>
